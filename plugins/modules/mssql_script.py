...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("script", resource_id)
        elif module.params.get("name"):
            candidates = client.list("script", {dict(name=module.params.get("name", ""))})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, script=existing)
            result = client.update("script", resource_id or existing.get("id", ""), module.params)

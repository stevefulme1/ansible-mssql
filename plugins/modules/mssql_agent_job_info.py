...
DOCUMENTATION = r"""
---
module: mssql_agent_job_info
short_description: Retrieve agent job information
description:
    - Retrieve details about agent jobs.
    - Read-only module.
version_added: "1.0.0"
author:
...
EXAMPLES = r"""
- name: List all agent jobs
  stevefulme1.mssql.mssql_agent_job_info:
    host: api.example.com
  register: result

- name: Get a specific agent job
  stevefulme1.mssql.mssql_agent_job_info:
    host: api.example.com
    job_name: "example-id"
  register: result
...
def main():
    ...
    if resource_id:
        result = client.get("agent_job", resource_id)
        ...
        resources = client.list("agent_job", module.params)

    module.exit_json(changed=False, agent_jobs=resources)

...
description:
    - Manage Azure SQL Server instances.
version_added: "1.0.0"
author:
...
options:
    ...
    host:
        ...
        required: true
    server_name:
        description: Unique identifier of the sql server.
        type: str
    name:
...
- name: Create a sql server
  stevefulme1.mssql.azure_sql_server:
    host: api.example.com
    name: my-sql-server
    state: present

- name: Delete a sql server
  stevefulme1.mssql.azure_sql_server:
    host: api.example.com
...
def main():
    ...
    if state == "present":
        ...
        if resource_id:
            ...
            candidates = client.list("sql_server", {dict(name=module.params.get("name", ""))})
            if candidates:

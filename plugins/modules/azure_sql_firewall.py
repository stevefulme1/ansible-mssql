...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("sql_firewall", resource_id)
        elif module.params.get("name"):
            candidates = client.list("sql_firewall", {dict(name=module.params.get("name", ""))})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, sql_firewall=existing)
            result = client.update("sql_firewall", resource_id or existing.get("id", ""), module.params)

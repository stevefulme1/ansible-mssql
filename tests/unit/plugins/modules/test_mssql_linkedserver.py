# -*- coding: utf-8 -*-
"""Comprehensive unit tests for mssql_linkedserver module."""
...
from ansible_collections.stevefulme1.mssql.plugins.modules import mssql_linkedserver
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(mssql_linkedserver, "DOCUMENTATION")
        assert len(mssql_linkedserver.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "mssql_linkedserver" in mssql_linkedserver.DOCUMENTATION or "linkedserver" in mssql_linkedserver.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in mssql_linkedserver.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in mssql_linkedserver.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(mssql_linkedserver, "EXAMPLES")
        assert len(mssql_linkedserver.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.mssql" in mssql_linkedserver.EXAMPLES

    def test_return_exists(self):
        assert hasattr(mssql_linkedserver, "RETURN")
        assert len(mssql_linkedserver.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_returns_resource(self, mock_client):
        mock_client.create.return_value = {"id": "new-1", "name": "test-linkedserver"}
        result = mock_client.create("linkedserver", {"name": "test-linkedserver"})
        assert result["id"] == "new-1"

    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-linkedserver", "description": "test", "enabled": True}
        mock_client.create.return_value = {"id": "new-2", **params}
        result = mock_client.create("linkedserver", params)
        assert result["name"] == "full-linkedserver"

    def test_create_sets_changed(self, mock_client):
        result = {"changed": True, "linkedserver": {"id": "1"}}
        assert result["changed"] is True

    def test_create_idempotent(self, mock_client_existing):
        existing = mock_client_existing.get("linkedserver", "123")
        assert existing is not None
        result = {"changed": False, "linkedserver": existing}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_client_existing):
        mock_client_existing.delete("linkedserver", "123")
        mock_client_existing.delete.assert_called_once_with("linkedserver", "123")
...
class TestGet:
    ...
    def test_get_existing(self, mock_client_existing):
        result = mock_client_existing.get("linkedserver", "123")
        assert result["id"] == "123"

    def test_get_nonexistent(self, mock_client):
        result = mock_client.get("linkedserver", "nonexistent")
    ...
    def test_get_returns_all_fields(self, mock_client):
        ...
        result = mock_client.get("linkedserver", "123")
...
class TestUpdate:
    ...
    def test_update_returns_updated(self, mock_client):
        mock_client.update.return_value = {"id": "123", "name": "updated-linkedserver"}
        result = mock_client.update("linkedserver", "123", {"name": "updated-linkedserver"})
        assert result["name"] == "updated-linkedserver"
    ...
    def test_update_with_changes(self, mock_client_existing):
        ...
        result = {"changed": True, "linkedserver": mock_client_existing.update("linkedserver", "123", {"name": "changed"})}
        assert result["changed"] is True
...
class TestList:
    ...
    def test_list_returns_items(self, mock_client):
        ...
        result = mock_client.list("linkedserver")
        assert len(result) == 2

    def test_list_empty(self, mock_client):
        assert len(mock_client.list("linkedserver")) == 0
...
class TestCheckMode:
    ...
    def test_check_mode_create(self, mock_module_check_mode, mock_client):
        if mock_module_check_mode.check_mode:
            result = {"changed": True, "linkedserver": {}}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_client):
        ...
        with pytest.raises(ConnectionError):
            mock_client.get("linkedserver", "123")
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("linkedserver", {"bad": "param"})
...
class TestReturnValues:
    ...
    def test_return_has_resource(self):
        result = {"changed": True, "linkedserver": {"id": "1", "name": "test"}}
        assert "linkedserver" in result

    def test_return_resource_has_id(self):
        result = {"changed": True, "linkedserver": {"id": "abc-123"}}
        assert "id" in result["linkedserver"]
    ...
    def test_return_unchanged_noop(self):
        result = {"changed": False, "linkedserver": {"id": "1"}}
        assert result["changed"] is False

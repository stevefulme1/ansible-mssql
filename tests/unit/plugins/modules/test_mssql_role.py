# -*- coding: utf-8 -*-
"""Comprehensive unit tests for mssql_role module."""
...
from ansible_collections.stevefulme1.mssql.plugins.modules import mssql_role
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(mssql_role, "DOCUMENTATION")
        assert len(mssql_role.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "mssql_role" in mssql_role.DOCUMENTATION or "role" in mssql_role.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in mssql_role.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in mssql_role.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(mssql_role, "EXAMPLES")
        assert len(mssql_role.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.mssql" in mssql_role.EXAMPLES

    def test_return_exists(self):
        assert hasattr(mssql_role, "RETURN")
        assert len(mssql_role.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_with_all_params(self, mock_client):
        params = {"name": "full-role", "description": "test", "enabled": True}
...
class TestErrorHandling:
    ...
    def test_invalid_params(self, mock_client):
        ...
        with pytest.raises(ValueError):
            mock_client.create("role", {"bad": "param"})
